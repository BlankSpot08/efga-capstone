...
// Page object
$UserAccountEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { user_account: currentTable } });
var currentForm, currentPageID;
var fuser_accountedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fuser_accountedit = new ew.Form("fuser_accountedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fuser_accountedit;

    // Add fields
    var fields = currentTable.fields;
    fuser_accountedit.addFields([
    ...
    // Form_CustomValidate
    fuser_accountedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fuser_accountedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fuser_accountedit.lists._userlevel = <?= $Page->_userlevel->toClientList($Page) ?>;
    loadjs.done("fuser_accountedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fuser_accountedit" id="fuser_accountedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_user_account_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="user_account" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_user_account_id">
<span<?= $Page->id->viewAttributes() ?>>
...
<input type="hidden" data-table="user_account" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->FormValue) ?>">
<?php } ?>
</div></div>
    </div>
<?php } ?>
...
<span id="el_user_account__userlevel">
    <select
        ...
        data-select2-id="fuser_accountedit_x__userlevel"
...
<script>
loadjs.ready("fuser_accountedit", function() {
    var options = { name: "x__userlevel", selectId: "fuser_accountedit_x__userlevel" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fuser_accountedit.lists._userlevel.lookupOptions.length) {
        options.data = { id: "x__userlevel", form: "fuser_accountedit" };
    } else {
        options.ajax = { id: "x__userlevel", form: "fuser_accountedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
