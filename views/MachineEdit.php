...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmachineedit = new ew.Form("fmachineedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fmachineedit;

    // Add fields
    var fields = currentTable.fields;
    fmachineedit.addFields([
    ...
    // Form_CustomValidate
    fmachineedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmachineedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmachineedit.lists.machine_category_id = <?= $Page->machine_category_id->toClientList($Page) ?>;
    fmachineedit.lists.brand_id = <?= $Page->brand_id->toClientList($Page) ?>;
    loadjs.done("fmachineedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmachineedit" id="fmachineedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_machine_machine_category_id">
    <select
        ...
        data-select2-id="fmachineedit_x_machine_category_id"
        data-table="machine"
...
<script>
loadjs.ready("fmachineedit", function() {
    var options = { name: "x_machine_category_id", selectId: "fmachineedit_x_machine_category_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmachineedit.lists.machine_category_id.lookupOptions.length) {
        options.data = { id: "x_machine_category_id", form: "fmachineedit" };
    } else {
        options.ajax = { id: "x_machine_category_id", form: "fmachineedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_machine_brand_id">
    <select
        ...
        data-select2-id="fmachineedit_x_brand_id"
        data-table="machine"
...
<script>
loadjs.ready("fmachineedit", function() {
    var options = { name: "x_brand_id", selectId: "fmachineedit_x_brand_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmachineedit.lists.brand_id.lookupOptions.length) {
        options.data = { id: "x_brand_id", form: "fmachineedit" };
    } else {
        options.ajax = { id: "x_brand_id", form: "fmachineedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    ...
    <input type="hidden" data-table="machine" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
