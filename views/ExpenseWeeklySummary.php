...
// Page object
$ExpenseWeeklySummary = &$Page;
?>
<?php if (!$Page->isExport() && !$Page->DrillDown && !$DashboardReport) { ?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { ExpenseWeekly: currentTable } });
...
<!-- Report grid (begin) -->
<div id="gmp_ExpenseWeekly" class="<?= ResponsiveTableClass() ?>card-body ew-grid-middle-panel">
<table class="<?= $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
    <tr class="ew-table-header">
<?php if ($Page->SUMaemp_amount->Visible) { ?>
    <th data-name="SUMaemp_amount" class="<?= $Page->SUMaemp_amount->headerCellClass() ?>"><div class="ExpenseWeekly_SUMaemp_amount"><?= $Page->renderFieldHeader($Page->SUMaemp_amount) ?></div></th>
<?php } ?>
    </tr>
</thead>
<tbody>
...
if (!$DashboardReport) {
    ...
    if (($Page->isExport("print") || $Page->isExport("pdf") || $Page->isExport("email") || $Page->isExport("excel") && Config("USE_PHPEXCEL") || $Page->isExport("word") && Config("USE_PHPWORD")) && $Page->ExportChartPageBreak) {
        ...
        $Page->ExpenseWeekly->PageBreakType = "before"; // Page break type
        $Page->ExpenseWeekly->PageBreak = $Page->ExportChartPageBreak;
        $Page->ExpenseWeekly->PageBreakContent = $Page->PageBreakContent;
    }

    // Set up chart drilldown
    $Page->ExpenseWeekly->DrillDownInPanel = $Page->DrillDownInPanel;
    $Page->ExpenseWeekly->render("ew-chart-bottom");
}
?>
<?php if (!$DashboardReport && !$Page->isExport("email") && !$Page->DrillDown && $Page->ExpenseWeekly->hasData()) { ?>
