...
// Page object
$EmpExpenseSubcategoryAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { emp_expense_subcategory: currentTable } });
var currentForm, currentPageID;
var femp_expense_subcategoryadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    femp_expense_subcategoryadd = new ew.Form("femp_expense_subcategoryadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = femp_expense_subcategoryadd;

    // Add fields
    var fields = currentTable.fields;
    femp_expense_subcategoryadd.addFields([
    ...
    // Form_CustomValidate
    femp_expense_subcategoryadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    femp_expense_subcategoryadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    femp_expense_subcategoryadd.lists.expCategory = <?= $Page->expCategory->toClientList($Page) ?>;
    loadjs.done("femp_expense_subcategoryadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="femp_expense_subcategoryadd" id="femp_expense_subcategoryadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_emp_expense_subcategory_expCategory">
    <select
        ...
        data-select2-id="femp_expense_subcategoryadd_x_expCategory"
...
<script>
loadjs.ready("femp_expense_subcategoryadd", function() {
    var options = { name: "x_expCategory", selectId: "femp_expense_subcategoryadd_x_expCategory" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (femp_expense_subcategoryadd.lists.expCategory.lookupOptions.length) {
        options.data = { id: "x_expCategory", form: "femp_expense_subcategoryadd" };
    } else {
        options.ajax = { id: "x_expCategory", form: "femp_expense_subcategoryadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
