...
// Page object
$ExpenseTodaySummary = &$Page;
?>
<?php if (!$Page->isExport() && !$Page->DrillDown && !$DashboardReport) { ?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { ExpenseToday: currentTable } });
...
<!-- Report grid (begin) -->
<div id="gmp_ExpenseToday" class="<?= ResponsiveTableClass() ?>card-body ew-grid-middle-panel">
<table class="<?= $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
    <tr class="ew-table-header">
<?php if ($Page->SUMaemp_amount->Visible) { ?>
    <th data-name="SUMaemp_amount" class="<?= $Page->SUMaemp_amount->headerCellClass() ?>"><div class="ExpenseToday_SUMaemp_amount"><?= $Page->renderFieldHeader($Page->SUMaemp_amount) ?></div></th>
<?php } ?>
    </tr>
</thead>
<tbody>
...
if (!$DashboardReport) {
    ...
    if (($Page->isExport("print") || $Page->isExport("pdf") || $Page->isExport("email") || $Page->isExport("excel") && Config("USE_PHPEXCEL") || $Page->isExport("word") && Config("USE_PHPWORD")) && $Page->ExportChartPageBreak) {
        ...
        $Page->ExpenseToday->PageBreakType = "before"; // Page break type
        $Page->ExpenseToday->PageBreak = $Page->ExportChartPageBreak;
        $Page->ExpenseToday->PageBreakContent = $Page->PageBreakContent;
    }

    // Set up chart drilldown
    $Page->ExpenseToday->DrillDownInPanel = $Page->DrillDownInPanel;
    $Page->ExpenseToday->render("ew-chart-bottom");
}
?>
<?php if (!$DashboardReport && !$Page->isExport("email") && !$Page->DrillDown && $Page->ExpenseToday->hasData()) { ?>
