...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    femployeeedit = new ew.Form("femployeeedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = femployeeedit;
    ...
    femployeeedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    femployeeedit.lists.officeDepartment = <?= $Page->officeDepartment->toClientList($Page) ?>;
    femployeeedit.lists.empPosition = <?= $Page->empPosition->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="femployeeedit" id="femployeeedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="employee" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->FormValue) ?>">
<?php } ?>
</div></div>
    </div>
<?php } ?>
...
loadjs.ready(["femployeeedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("femployeeedit", "x_dateOfBirth", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php $Page->officeDepartment->EditAttrs->prepend("onchange", "ew.updateOptions.call(this);"); ?>
    <select
        ...
        data-select2-id="femployeeedit_x_officeDepartment"
        data-table="employee"
...
<script>
loadjs.ready("femployeeedit", function() {
    var options = { name: "x_officeDepartment", selectId: "femployeeedit_x_officeDepartment" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (femployeeedit.lists.officeDepartment.lookupOptions.length) {
        options.data = { id: "x_officeDepartment", form: "femployeeedit" };
    } else {
        options.ajax = { id: "x_officeDepartment", form: "femployeeedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_employee_empPosition">
    <select
        ...
        data-select2-id="femployeeedit_x_empPosition"
        data-table="employee"
...
<script>
loadjs.ready("femployeeedit", function() {
    var options = { name: "x_empPosition", selectId: "femployeeedit_x_empPosition" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (femployeeedit.lists.empPosition.lookupOptions.length) {
        options.data = { id: "x_empPosition", form: "femployeeedit" };
    } else {
        options.ajax = { id: "x_empPosition", form: "femployeeedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
