...
// Page object
$ExpenseYesterdaySummary = &$Page;
?>
<?php if (!$Page->isExport() && !$Page->DrillDown && !$DashboardReport) { ?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { ExpenseYesterday: currentTable } });
...
<!-- Report grid (begin) -->
<div id="gmp_ExpenseYesterday" class="<?= ResponsiveTableClass() ?>card-body ew-grid-middle-panel">
<table class="<?= $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
    <tr class="ew-table-header">
<?php if ($Page->SUMaemp_amount->Visible) { ?>
    <th data-name="SUMaemp_amount" class="<?= $Page->SUMaemp_amount->headerCellClass() ?>"><div class="ExpenseYesterday_SUMaemp_amount"><?= $Page->renderFieldHeader($Page->SUMaemp_amount) ?></div></th>
<?php } ?>
    </tr>
</thead>
<tbody>
...
<?php if ($Page->SUMaemp_amount->Visible) { ?>
        <td data-field="SUMaemp_amount"<?= $Page->SUMaemp_amount->cellAttributes() ?>>
...
<?php if ($Page->ShowCompactSummaryFooter) { ?>
    <tr<?= $Page->rowAttributes(); ?>><td colspan="<?= ($Page->GroupColumnCount + $Page->DetailColumnCount) ?>"><?= $Language->phrase("RptPageSummary") ?> <span class="ew-summary-count">(<span class="ew-aggregate-caption"><?= $Language->phrase("RptCnt") ?></span><?= $Language->phrase("AggregateEqual") ?><span class="ew-aggregate-value"><?= FormatNumber($Page->PageTotalCount, Config("DEFAULT_NUMBER_FORMAT")) ?></span>)</span></td></tr>
<?php } else { ?>
...
if (!$DashboardReport) {
    ...
    if (($Page->isExport("print") || $Page->isExport("pdf") || $Page->isExport("email") || $Page->isExport("excel") && Config("USE_PHPEXCEL") || $Page->isExport("word") && Config("USE_PHPWORD")) && $Page->ExportChartPageBreak) {
        ...
        $Page->ExpenseYesterday->PageBreakType = "before"; // Page break type
        $Page->ExpenseYesterday->PageBreak = $Page->ExportChartPageBreak;
        $Page->ExpenseYesterday->PageBreakContent = $Page->PageBreakContent;
    }

    // Set up chart drilldown
    $Page->ExpenseYesterday->DrillDownInPanel = $Page->DrillDownInPanel;
    $Page->ExpenseYesterday->render("ew-chart-bottom");
}
?>
<?php if (!$DashboardReport && !$Page->isExport("email") && !$Page->DrillDown && $Page->ExpenseYesterday->hasData()) { ?>
