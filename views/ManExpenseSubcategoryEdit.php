...
// Page object
$ManExpenseSubcategoryEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { man_expense_subcategory: currentTable } });
var currentForm, currentPageID;
var fman_expense_subcategoryedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fman_expense_subcategoryedit = new ew.Form("fman_expense_subcategoryedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fman_expense_subcategoryedit;

    // Add fields
    var fields = currentTable.fields;
    fman_expense_subcategoryedit.addFields([
    ...
    // Form_CustomValidate
    fman_expense_subcategoryedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fman_expense_subcategoryedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fman_expense_subcategoryedit.lists.expCategory = <?= $Page->expCategory->toClientList($Page) ?>;
    loadjs.done("fman_expense_subcategoryedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fman_expense_subcategoryedit" id="fman_expense_subcategoryedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_man_expense_subcategory_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="man_expense_subcategory" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_man_expense_subcategory_id">
<span<?= $Page->id->viewAttributes() ?>>
...
<input type="hidden" data-table="man_expense_subcategory" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->FormValue) ?>">
<?php } ?>
</div></div>
    </div>
<?php } ?>
...
<span id="el_man_expense_subcategory_expCategory">
    <select
        ...
        data-select2-id="fman_expense_subcategoryedit_x_expCategory"
...
<script>
loadjs.ready("fman_expense_subcategoryedit", function() {
    var options = { name: "x_expCategory", selectId: "fman_expense_subcategoryedit_x_expCategory" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fman_expense_subcategoryedit.lists.expCategory.lookupOptions.length) {
        options.data = { id: "x_expCategory", form: "fman_expense_subcategoryedit" };
    } else {
        options.ajax = { id: "x_expCategory", form: "fman_expense_subcategoryedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
