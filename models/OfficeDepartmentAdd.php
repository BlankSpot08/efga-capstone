 ...
 * Page class
 */
class OfficeDepartmentAdd extends OfficeDepartment
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "OfficeDepartmentAdd";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "OfficeDepartmentView") {
                        $row["view"] = "1";
                    }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("OfficeDepartmentList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "OfficeDepartmentList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "OfficeDepartmentView") {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$this->officeDepartment->Raw) {
                $this->officeDepartment->CurrentValue = HtmlDecode($this->officeDepartment->CurrentValue);
            }
            $this->officeDepartment->EditValue = HtmlEncode($this->officeDepartment->CurrentValue);
    ...
    {
        global $Language, $Security;
        if ($this->officeDepartment->CurrentValue != "") { // Check field with unique index
            $filter = "(`officeDepartment` = '" . AdjustSql($this->officeDepartment->CurrentValue, $this->Dbid) . "')";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->officeDepartment->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->officeDepartment->CurrentValue, $idxErrMsg);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("OfficeDepartmentList"), "", $this->TableVar, true);
