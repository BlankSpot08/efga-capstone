 ...
 * Page class
 */
class MachineBrandAdd extends MachineBrand
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MachineBrandAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("MachineBrandList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "MachineBrandList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "MachineBrandView") {
    ...
    {
        global $Language, $Security;
        if ($this->brand->CurrentValue != "") { // Check field with unique index
            $filter = "(`brand` = '" . AdjustSql($this->brand->CurrentValue, $this->Dbid) . "')";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->brand->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->brand->CurrentValue, $idxErrMsg);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("MachineBrandList"), "", $this->TableVar, true);
