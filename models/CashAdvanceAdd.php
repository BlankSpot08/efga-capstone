 ...
 * Page class
 */
class CashAdvanceAdd extends CashAdvance
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CashAdvanceAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "CashAdvanceView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->expSubcategory_id->CurrentValue = null;
        $this->expSubcategory_id->OldValue = $this->expSubcategory_id->CurrentValue;
        $this->budget_id->CurrentValue = null;
        $this->budget_id->OldValue = $this->budget_id->CurrentValue;
    ...
    protected function newRow()
    {
        ...
        $row['expSubcategory_id'] = $this->expSubcategory_id->CurrentValue;
        $row['budget_id'] = $this->budget_id->CurrentValue;
        $row['machine_id'] = $this->machine_id->CurrentValue;
        $row['dateReceived'] = $this->dateReceived->CurrentValue;
        ...
        $row['used'] = $this->used->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $curVal = trim(strval($this->budget_id->CurrentValue));
            if ($curVal != "") {
                $this->budget_id->ViewValue = $this->budget_id->lookupCacheOption($curVal);
            } else {
                $this->budget_id->ViewValue = $this->budget_id->Lookup !== null && is_array($this->budget_id->lookupOptions()) ? $curVal : null;
            }
            if ($this->budget_id->ViewValue !== null) { // Load from cache
                $this->budget_id->EditValue = array_values($this->budget_id->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                ...
                $sqlWrk = $this->budget_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCacheImpl($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                ...
                foreach ($arwrk as &$row) {
                    $row = $this->budget_id->Lookup->renderViewRow($row);
                }
                $this->budget_id->EditValue = $arwrk;
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // expCategory_id
        $this->expCategory_id->setDbValueDef($rsnew, $this->expCategory_id->CurrentValue, null, false);

        // expSubcategory_id
        $this->expSubcategory_id->setDbValueDef($rsnew, $this->expSubcategory_id->CurrentValue, null, false);

        // budget_id
        $this->budget_id->setDbValueDef($rsnew, $this->budget_id->CurrentValue, null, false);
