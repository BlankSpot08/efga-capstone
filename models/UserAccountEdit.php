 ...
 * Page class
 */
class UserAccountEdit extends UserAccount
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UserAccountEdit";
    ...
    protected function editRow()
    {
        ...
        if ($this->firstname->CurrentValue != "") { // Check field with unique index
            $filterChk = "(`firstname` = '" . AdjustSql($this->firstname->CurrentValue, $this->Dbid) . "')";
            $filterChk .= " AND NOT (" . $filter . ")";
            $this->CurrentFilter = $filterChk;
            $sqlChk = $this->getCurrentSql();
            $rsChk = $conn->executeQuery($sqlChk);
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['userlevel']); // userlevel
        return md5($hash);
    }
