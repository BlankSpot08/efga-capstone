...
/**
 * Table class for machine
 */
class Machine extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'machine';
        $this->TableName = 'machine';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`machine`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // machine_category_id
        $this->machine_category_id = new DbField(
            'machine',
            'machine',
            'x_machine_category_id',
            'machine_category_id',
            '`machine_category_id`',
            '`machine_category_id`',
            3,
        ...
        $this->machine_category_id->InputTextType = "text";
        $this->machine_category_id->Required = true; // Required field
        $this->machine_category_id->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->machine_category_id->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        switch ($CurrentLanguage) {
            case "en-US":
                $this->machine_category_id->Lookup = new Lookup('machine_category_id', 'machine_category', false, 'id', ["machine_category","","",""], [], [], [], [], [], [], '`id` ASC', '', "`machine_category`");
                break;
            default:
                $this->machine_category_id->Lookup = new Lookup('machine_category_id', 'machine_category', false, 'id', ["machine_category","","",""], [], [], [], [], [], [], '`id` ASC', '', "`machine_category`");
                break;
        }
        $this->machine_category_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['machine_category_id'] = &$this->machine_category_id;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->brand_id->Lookup = new Lookup('brand_id', 'machine_brand', false, 'id', ["brand","","",""], [], [], [], [], [], [], '`id` ASC', '', "`brand`");
                break;
            default:
                $this->brand_id->Lookup = new Lookup('brand_id', 'machine_brand', false, 'id', ["brand","","",""], [], [], [], [], [], [], '`id` ASC', '', "`brand`");
                break;
        ...
        $this->repairCount->InputTextType = "text";
        $this->repairCount->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['repairCount'] = &$this->repairCount;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`machine`";
    }
    ...
    public function getSqlSelectList() // Select for List page
    {
        ...
        $from = "(SELECT *, (SELECT `machine_category` FROM `machine_category` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id` = `machine`.`machine_category_id` LIMIT 1) AS `EV__machine_category_id`, (SELECT `brand` FROM `machine_brand` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id` = `machine`.`brand_id` LIMIT 1) AS `EV__brand_id` FROM `machine`)";
    ...
    protected function useVirtualFields()
    {
        ...
        if (ContainsString($orderBy, " " . $this->machine_category_id->VirtualExpression . " ")) {
            return true;
        }
        if (ContainsString($orderBy, " " . $this->brand_id->VirtualExpression . " ")) {
            return true;
        }
        return false;
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->machine_category_id->DbValue = $row['machine_category_id'];
        $this->brand_id->DbValue = $row['brand_id'];
        $this->model->DbValue = $row['model'];
        $this->repairCount->DbValue = $row['repairCount'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("MachineList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "MachineEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "MachineAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("MachineView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("MachineView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "MachineAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "MachineAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("MachineEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("MachineAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("MachineDelete", $this->getUrlParm());
    }
    ...
    public function renderListRow()
    {
        ...
        if ($this->photo->UseColorbox) {
            ...
            $this->photo->LinkAttrs["data-rel"] = "machine_x_photo";
    ...
    public function renderEditRow()
    {
        ...
        // repairCount
        $this->repairCount->setupEditAttributes();
        $this->repairCount->EditCustomAttributes = "";
        $this->repairCount->EditValue = $this->repairCount->CurrentValue;
        $this->repairCount->PlaceHolder = RemoveHtml($this->repairCount->caption());
        if (strval($this->repairCount->EditValue) != "" && is_numeric($this->repairCount->EditValue)) {
            $this->repairCount->EditValue = FormatNumber($this->repairCount->EditValue, null);
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->machine_category_id);
                    $doc->exportCaption($this->brand_id);
                    $doc->exportCaption($this->model);
                    $doc->exportCaption($this->repairCount);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->machine_category_id);
                        $doc->exportField($this->brand_id);
                        $doc->exportField($this->model);
                        $doc->exportField($this->repairCount);
