 ...
 * Page class
 */
class ManExpenseList extends ManExpense
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ManExpenseList";
    ...
    public $FormName = "fman_expenselist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ManExpenseAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ManExpenseDelete";
        $this->MultiUpdateUrl = "ManExpenseUpdate";
    ...
    public function run()
    {
        ...
        $this->expSubcategory->Visible = false;
        ...
        $this->consumption->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->dateFrom->AdvancedSearch->toJson(), ","); // Field dateFrom
        $filterList = Concat($filterList, $this->dateTo->AdvancedSearch->toJson(), ","); // Field dateTo
        $filterList = Concat($filterList, $this->consumption->AdvancedSearch->toJson(), ","); // Field consumption
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fman_expensesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field dateFrom
        $this->dateFrom->AdvancedSearch->SearchValue = @$filter["x_dateFrom"];
        $this->dateFrom->AdvancedSearch->SearchOperator = @$filter["z_dateFrom"];
        $this->dateFrom->AdvancedSearch->SearchCondition = @$filter["v_dateFrom"];
        $this->dateFrom->AdvancedSearch->SearchValue2 = @$filter["y_dateFrom"];
        $this->dateFrom->AdvancedSearch->SearchOperator2 = @$filter["w_dateFrom"];
        $this->dateFrom->AdvancedSearch->save();

        // Field dateTo
        $this->dateTo->AdvancedSearch->SearchValue = @$filter["x_dateTo"];
        $this->dateTo->AdvancedSearch->SearchOperator = @$filter["z_dateTo"];
        $this->dateTo->AdvancedSearch->SearchCondition = @$filter["v_dateTo"];
        $this->dateTo->AdvancedSearch->SearchValue2 = @$filter["y_dateTo"];
        $this->dateTo->AdvancedSearch->SearchOperator2 = @$filter["w_dateTo"];
        $this->dateTo->AdvancedSearch->save();

        // Field consumption
        $this->consumption->AdvancedSearch->SearchValue = @$filter["x_consumption"];
        $this->consumption->AdvancedSearch->SearchOperator = @$filter["z_consumption"];
        $this->consumption->AdvancedSearch->SearchCondition = @$filter["v_consumption"];
        $this->consumption->AdvancedSearch->SearchValue2 = @$filter["y_consumption"];
        $this->consumption->AdvancedSearch->SearchOperator2 = @$filter["w_consumption"];
        $this->consumption->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->dateFrom, $default, false); // dateFrom
        $this->buildSearchSql($where, $this->dateTo, $default, false); // dateTo
        $this->buildSearchSql($where, $this->consumption, $default, false); // consumption
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->dateFrom->AdvancedSearch->save(); // dateFrom
            $this->dateTo->AdvancedSearch->save(); // dateTo
            $this->consumption->AdvancedSearch->save(); // consumption
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->dateFrom->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->dateTo->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->consumption->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->dateFrom->AdvancedSearch->unsetSession();
        $this->dateTo->AdvancedSearch->unsetSession();
        $this->consumption->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->dateFrom->AdvancedSearch->load();
        $this->dateTo->AdvancedSearch->load();
        $this->consumption->AdvancedSearch->load();
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->date->setSort("");
                $this->dateFrom->setSort("");
                $this->dateTo->setSort("");
                $this->consumption->setSort("");
                $this->note->setSort("");
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fman_expenselist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fman_expenselist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fman_expensesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fman_expensesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fman_expenselist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // dateFrom
        if ($this->dateFrom->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->dateFrom->AdvancedSearch->SearchValue != "" || $this->dateFrom->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // dateTo
        if ($this->dateTo->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->dateTo->AdvancedSearch->SearchValue != "" || $this->dateTo->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // consumption
        if ($this->consumption->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->consumption->AdvancedSearch->SearchValue != "" || $this->consumption->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->receipt->UseColorbox) {
                ...
                $this->receipt->LinkAttrs["data-rel"] = "man_expense_x" . $this->RowCount . "_receipt";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fman_expenselist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fman_expensesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->expCategory->Visible || $this->expSubcategory->Visible;
    }
