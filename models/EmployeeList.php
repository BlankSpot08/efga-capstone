...
class EmployeeList extends Employee
{
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->officeDepartment->AdvancedSearch->toJson(), ","); // Field officeDepartment
        $filterList = Concat($filterList, $this->empPosition->AdvancedSearch->toJson(), ","); // Field empPosition
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "femployeesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field officeDepartment
        $this->officeDepartment->AdvancedSearch->SearchValue = @$filter["x_officeDepartment"];
        $this->officeDepartment->AdvancedSearch->SearchOperator = @$filter["z_officeDepartment"];
        $this->officeDepartment->AdvancedSearch->SearchCondition = @$filter["v_officeDepartment"];
        $this->officeDepartment->AdvancedSearch->SearchValue2 = @$filter["y_officeDepartment"];
        $this->officeDepartment->AdvancedSearch->SearchOperator2 = @$filter["w_officeDepartment"];
        $this->officeDepartment->AdvancedSearch->save();

        // Field empPosition
        $this->empPosition->AdvancedSearch->SearchValue = @$filter["x_empPosition"];
        $this->empPosition->AdvancedSearch->SearchOperator = @$filter["z_empPosition"];
        $this->empPosition->AdvancedSearch->SearchCondition = @$filter["v_empPosition"];
        $this->empPosition->AdvancedSearch->SearchValue2 = @$filter["y_empPosition"];
        $this->empPosition->AdvancedSearch->SearchOperator2 = @$filter["w_empPosition"];
        $this->empPosition->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->officeDepartment); // officeDepartment
            $this->updateSort($this->empPosition); // empPosition
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->officeDepartment->setSort("");
                $this->empPosition->setSort("");
            }
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"employee\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"femployeelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"femployeelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("employee_id", $this->createColumnOption("employee_id"));
            $option->add("lastname", $this->createColumnOption("lastname"));
            $option->add("firstname", $this->createColumnOption("firstname"));
            $option->add("middlename", $this->createColumnOption("middlename"));
            $option->add("dateOfBirth", $this->createColumnOption("dateOfBirth"));
            $option->add("picture", $this->createColumnOption("picture"));
            $option->add("address", $this->createColumnOption("address"));
            $option->add("contactNo", $this->createColumnOption("contactNo"));
            $option->add("officeDepartment", $this->createColumnOption("officeDepartment"));
            $option->add("empPosition", $this->createColumnOption("empPosition"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"femployeesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"femployeesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="femployeelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->picture->UseColorbox) {
                ...
                $this->picture->LinkAttrs["data-rel"] = "employee_x" . $this->RowCount . "_picture";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="femployeelist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"femployeesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
