 ...
 * Page class
 */
class PendingExpenseEdit extends PendingExpense
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PendingExpenseEdit";
    ...
    public function run()
    {
        ...
        $this->receiptNumber->Required = false;
        $this->expCategory_id->Required = false;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("PendingExpenseList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "PendingExpenseList") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->dateTrans->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->dateTrans->setFormValue($val);
            }
        ...
        if (!$this->dateClosed->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->dateClosed->setFormValue($val, true, $validate);
            }
    ...
    protected function validateForm()
    {
        ...
        if (!CheckDate($this->dateClosed->FormValue, $this->dateClosed->formatPattern())) {
            $this->dateClosed->addErrorMessage($this->dateClosed->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // dateClosed
            $this->dateClosed->setDbValueDef($rsnew, UnFormatDateTime($this->dateClosed->CurrentValue, $this->dateClosed->formatPattern()), null, $this->dateClosed->ReadOnly);

            // float_status
            $this->float_status->setDbValueDef($rsnew, $this->float_status->CurrentValue, null, $this->float_status->ReadOnly);
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['dateClosed']); // dateClosed
        $hash .= GetFieldHash($row['float_status']); // float_status
        return md5($hash);
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PendingExpenseList"), "", $this->TableVar, true);
        $pageId = "edit";
