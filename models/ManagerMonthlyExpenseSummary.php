 ...
 * Page class
 */
class ManagerMonthlyExpenseSummary extends ManagerMonthlyExpense
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'Manager Monthly Expense';

    // Page object name
    public $PageObjName = "ManagerMonthlyExpenseSummary";
    ...
    public function __construct()
    {
        ...
        // Table object (ManagerMonthlyExpense)
        if (!isset($GLOBALS["ManagerMonthlyExpense"]) || get_class($GLOBALS["ManagerMonthlyExpense"]) == PROJECT_NAMESPACE . "ManagerMonthlyExpense") {
            $GLOBALS["ManagerMonthlyExpense"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Manager Monthly Expense');
        }
    ...
    public function loadRowValues($record)
    {
        ...
        $this->consumption->setDbValue($record['consumption']);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_TOTAL && $this->RowTotalSubType == ROWTOTAL_FOOTER && $this->RowTotalType == ROWTOTAL_PAGE) {
            ...
            $this->consumption->getSum($records);
            ...
            if ($rsagg) {
                ...
                $this->consumption->Count = $this->TotalCount;
                $this->consumption->SumValue = $rsagg["sum_consumption"];
            ...
            if (!$hasCount || !$hasSummary) {
                ...
                $this->consumption->getSum($this->DetailRecords);
            }
        }
        ...
        if ($this->RowType == ROWTYPE_SEARCH) { // Search row
            ...
            if ($this->RowTotalType == ROWTOTAL_GROUP && $this->RowGroupLevel >= 2) {
                $this->RowAttrs["data-group-2"] = $this->date->groupValue(); // Set up group attribute 2
            }
            ...
            // date
            $this->date->GroupViewValue = $this->date->groupValue();
            $this->date->GroupViewValue = FormatDateTime($this->date->GroupViewValue, $this->date->formatPattern());
            $this->date->CellCssClass = ($this->RowGroupLevel == 2 ? "ew-rpt-grp-summary-2" : "ew-rpt-grp-field-2");
            $this->date->ViewCustomAttributes = "";
            $this->date->GroupViewValue = DisplayGroupValue($this->date, $this->date->GroupViewValue);
            ...
            // consumption
            $this->consumption->SumViewValue = $this->consumption->SumValue;
            $this->consumption->SumViewValue = FormatNumber($this->consumption->SumViewValue, $this->consumption->formatPattern());
            $this->consumption->ViewCustomAttributes = "";
            $this->consumption->CellAttrs["class"] = ($this->RowTotalType == ROWTOTAL_PAGE || $this->RowTotalType == ROWTOTAL_GRAND) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" . $this->RowGroupLevel;
            ...
            $this->date->GroupViewValue = $this->date->groupValue();
            $this->date->GroupViewValue = FormatDateTime($this->date->GroupViewValue, $this->date->formatPattern());
            $this->date->CellCssClass = "ew-rpt-grp-field-2";
            $this->date->ViewCustomAttributes = "";
            $this->date->GroupViewValue = DisplayGroupValue($this->date, $this->date->GroupViewValue);
            if (!$this->date->LevelBreak) {
                $this->date->GroupViewValue = "";
            } else {
                $this->date->LevelBreak = false;
            }
            ...
            $this->consumption->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            $this->consumption->ViewCustomAttributes = "";
        ...
        if ($this->RowType == ROWTYPE_TOTAL) {
            ...
            // date
            $currentValue = $this->date->GroupViewValue;
            $viewValue = &$this->date->GroupViewValue;
            $viewAttrs = &$this->date->ViewAttrs;
            $cellAttrs = &$this->date->CellAttrs;
            $hrefValue = &$this->date->HrefValue;
            $linkAttrs = &$this->date->LinkAttrs;
            $this->cellRendered($this->date, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
            ...
            // consumption
            $currentValue = $this->consumption->SumValue;
            $viewValue = &$this->consumption->SumViewValue;
            $viewAttrs = &$this->consumption->ViewAttrs;
            $cellAttrs = &$this->consumption->CellAttrs;
            $hrefValue = &$this->consumption->HrefValue;
            $linkAttrs = &$this->consumption->LinkAttrs;
            $this->cellRendered($this->consumption, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
        } else {
            // expCategory
            ...
            // date
            $currentValue = $this->date->groupValue();
            $viewValue = &$this->date->GroupViewValue;
            $viewAttrs = &$this->date->ViewAttrs;
            $cellAttrs = &$this->date->CellAttrs;
            $hrefValue = &$this->date->HrefValue;
            ...
            // consumption
            $currentValue = $this->consumption->CurrentValue;
            $viewValue = &$this->consumption->ViewValue;
            $viewAttrs = &$this->consumption->ViewAttrs;
            $cellAttrs = &$this->consumption->CellAttrs;
            $hrefValue = &$this->consumption->HrefValue;
            $linkAttrs = &$this->consumption->LinkAttrs;
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fManagerMonthlyExpensesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fManagerMonthlyExpensesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    protected function getSort()
    {
        ...
        // Check for an Order parameter
            ...
            $this->updateSort($this->consumption); // consumption
