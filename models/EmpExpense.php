...
/**
 * Table class for emp_expense
 */
class EmpExpense extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'emp_expense';
        $this->TableName = 'emp_expense';
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->dateTrans->Lookup = new Lookup('dateTrans', 'emp_expense', true, 'dateTrans', ["dateTrans","","",""], [], [], [], [], [], [], '', '', "");
                break;
            default:
                $this->dateTrans->Lookup = new Lookup('dateTrans', 'emp_expense', true, 'dateTrans', ["dateTrans","","",""], [], [], [], [], [], [], '', '', "");
                break;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->submittedBy->Lookup = new Lookup('submittedBy', 'emp_expense', true, 'submittedBy', ["submittedBy","","",""], [], [], [], [], [], [], '', '', "");
                break;
            default:
                $this->submittedBy->Lookup = new Lookup('submittedBy', 'emp_expense', true, 'submittedBy', ["submittedBy","","",""], [], [], [], [], [], [], '', '', "");
                break;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->status->Lookup = new Lookup('status', 'emp_expense', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
                break;
            default:
                $this->status->Lookup = new Lookup('status', 'emp_expense', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
                break;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->float_status->Lookup = new Lookup('float_status', 'emp_expense', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
                break;
            default:
                $this->float_status->Lookup = new Lookup('float_status', 'emp_expense', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
                break;
    ...
    public function getSqlSelectList() // Select for List page
    {
        ...
        $from = "(SELECT *, (SELECT CONCAT(COALESCE(`machine_category_id`, ''),'" . ValueSeparator(1, $this->machine_id) . "',COALESCE(`brand_id`,''),'" . ValueSeparator(2, $this->machine_id) . "',COALESCE(`model`,'')) FROM `machine` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id` = `emp_expense`.`machine_id` LIMIT 1) AS `EV__machine_id` FROM `emp_expense`)";
    ...
    public function getSqlWhere() // Where
    {
        ...
        $this->DefaultFilter = "`submittedBy` = '" . CurrentUserName() . "' AND `float_status` = '0'";
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("EmpExpenseList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "EmpExpenseEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "EmpExpenseAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "EmpExpenseView";
            case Config("API_ADD_ACTION"):
                return "EmpExpenseAdd";
            case Config("API_EDIT_ACTION"):
                return "EmpExpenseEdit";
            case Config("API_DELETE_ACTION"):
                return "EmpExpenseDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("EmpExpenseView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("EmpExpenseView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "EmpExpenseAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "EmpExpenseAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("EmpExpenseEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("EmpExpenseAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("EmpExpenseDelete", $this->getUrlParm());
    }
    ...
    public function renderListRow()
    {
        ...
        if ($this->receipt->UseColorbox) {
            ...
            $this->receipt->LinkAttrs["data-rel"] = "emp_expense_x_receipt";
    ...
    {
        //Log("Row Inserted");
        calculateCashFloat($rsnew['cashAdvance_id'], $rsnew['id'], $rsnew['amount']);
        settingEmployeeExpenseUsed($rsnew['cashAdvance_id']);
        settingEmployeeCategoryId($rsnew['id'], $rsnew['cashAdvance_id']);
        settingRepairCount($rsnew['machine_id']);
    }
    // Row Updating event
