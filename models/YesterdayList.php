 ...
 * Page class
 */
class YesterdayList extends Yesterday
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'yesterday';

    // Page object name
    public $PageObjName = "YesterdayList";
    ...
    public $FormName = "fyesterdaylist";
    ...
    public function __construct()
    {
        ...
        // Table object (yesterday)
        if (!isset($GLOBALS["yesterday"]) || get_class($GLOBALS["yesterday"]) == PROJECT_NAMESPACE . "yesterday") {
            $GLOBALS["yesterday"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "YesterdayAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "YesterdayDelete";
        $this->MultiUpdateUrl = "YesterdayUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'yesterday');
        }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->SUMaemp_amount); // SUM(a.emp_amount)
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fyesterdaylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fyesterdaylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("SUM(a.emp_amount)", $this->createColumnOption("SUM(a.emp_amount)"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fyesterdaysrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fyesterdaysrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fyesterdaylist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->SUMaemp_amount->setDbValue($row['SUM(a.emp_amount)']);
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            // SUM(a.emp_amount)
            $this->SUMaemp_amount->ViewValue = $this->SUMaemp_amount->CurrentValue;
            $this->SUMaemp_amount->ViewValue = FormatNumber($this->SUMaemp_amount->ViewValue, $this->SUMaemp_amount->formatPattern());
            $this->SUMaemp_amount->ViewCustomAttributes = "";

            // SUM(a.emp_amount)
            $this->SUMaemp_amount->LinkCustomAttributes = "";
            $this->SUMaemp_amount->HrefValue = "";
            $this->SUMaemp_amount->TooltipValue = "";
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fyesterdaylist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
