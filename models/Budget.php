...
class Budget extends DbTable
{
    ...
    public function __construct()
    {
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->expCategory->Lookup = new Lookup('expCategory', 'emp_expense_category', false, 'id', ["expCategory","","",""], [], ["x_expSubcategory"], [], [], [], [], '`id` ASC', '', "`expCategory`");
                break;
            default:
                $this->expCategory->Lookup = new Lookup('expCategory', 'emp_expense_category', false, 'id', ["expCategory","","",""], [], ["x_expSubcategory"], [], [], [], [], '`id` ASC', '', "`expCategory`");
                break;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->expSubcategory->Lookup = new Lookup('expSubcategory', 'emp_expense_subcategory', false, 'id', ["expSubcategory","","",""], ["x_expCategory"], [], ["expCategory"], ["x_expCategory"], [], [], '`id` ASC', '', "`expSubcategory`");
                break;
            default:
                $this->expSubcategory->Lookup = new Lookup('expSubcategory', 'emp_expense_subcategory', false, 'id', ["expSubcategory","","",""], ["x_expCategory"], [], ["expCategory"], ["x_expCategory"], [], [], '`id` ASC', '', "`expSubcategory`");
                break;
    ...
    public function getSqlSelectList() // Select for List page
    {
        ...
        $from = "(SELECT *, (SELECT `expCategory` FROM `emp_expense_category` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id` = `budget`.`expCategory` LIMIT 1) AS `EV__expCategory`, (SELECT `expSubcategory` FROM `emp_expense_subcategory` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id` = `budget`.`expSubcategory` LIMIT 1) AS `EV__expSubcategory` FROM `budget`)";
    ...
    protected function useVirtualFields()
    {
        ...
        if (ContainsString($orderBy, " " . $this->expSubcategory->VirtualExpression . " ")) {
            return true;
        }
        return false;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("BudgetList");
    }
    ...
    {
        global $Language;
        if ($pageName == "BudgetView") {
            return $Language->phrase("View");
        } elseif ($pageName == "BudgetEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "BudgetAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("BudgetView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("BudgetView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "BudgetAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "BudgetAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("BudgetEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BudgetAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("BudgetDelete", $this->getUrlParm());
    }
