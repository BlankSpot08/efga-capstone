 ...
 * Page class
 */
class Machinerepairhistory2List extends Machinerepairhistory2
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'machinerepairhistory';

    // Page object name
    public $PageObjName = "Machinerepairhistory2List";
    ...
    public $FormName = "fmachinerepairhistory2list";
    ...
    public function __construct()
    {
        ...
        // Table object (machinerepairhistory2)
        if (!isset($GLOBALS["machinerepairhistory2"]) || get_class($GLOBALS["machinerepairhistory2"]) == PROJECT_NAMESPACE . "machinerepairhistory2") {
            $GLOBALS["machinerepairhistory2"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "Machinerepairhistory2Add";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "Machinerepairhistory2Delete";
        $this->MultiUpdateUrl = "Machinerepairhistory2Update";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'machinerepairhistory');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("machinerepairhistory2");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->machine_category->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->machine_category->AdvancedSearch->toJson(), ","); // Field machine_category
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmachinerepairhistory2srch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->machine_category->AdvancedSearch->SearchValue = @$filter["x_machine_category"];
        $this->machine_category->AdvancedSearch->SearchOperator = @$filter["z_machine_category"];
        $this->machine_category->AdvancedSearch->SearchCondition = @$filter["v_machine_category"];
        $this->machine_category->AdvancedSearch->SearchValue2 = @$filter["y_machine_category"];
        $this->machine_category->AdvancedSearch->SearchOperator2 = @$filter["w_machine_category"];
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->machine_category, $default, true); // machine_category
        $this->buildSearchSql($where, $this->brand, $default, true); // brand
        $this->buildSearchSql($where, $this->model, $default, true); // model
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->machine_category->AdvancedSearch->save(); // machine_category
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->machine_category;
        $searchFlds[] = &$this->brand;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->machine_category->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmachinerepairhistory2list\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmachinerepairhistory2list\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("machine_category", $this->createColumnOption("machine_category"));
            $option->add("brand", $this->createColumnOption("brand"));
            $option->add("model", $this->createColumnOption("model"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmachinerepairhistory2srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmachinerepairhistory2srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmachinerepairhistory2list"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // machine_category
        if ($this->machine_category->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->machine_category->AdvancedSearch->SearchValue != "" || $this->machine_category->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // brand
        if ($this->brand->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->receipt->UseColorbox) {
                ...
                $this->receipt->LinkAttrs["data-rel"] = "machinerepairhistory2_x" . $this->RowCount . "_receipt";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmachinerepairhistory2list" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmachinerepairhistory2srch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
