 ...
 * Page class
 */
class EmployeePositionAdd extends EmployeePosition
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeePositionAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("EmployeePositionList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "EmployeePositionList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "EmployeePositionView") {
    ...
    {
        global $Language, $Security;
        if ($this->position->CurrentValue != "") { // Check field with unique index
            $filter = "(`position` = '" . AdjustSql($this->position->CurrentValue, $this->Dbid) . "')";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->position->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->position->CurrentValue, $idxErrMsg);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("EmployeePositionList"), "", $this->TableVar, true);
