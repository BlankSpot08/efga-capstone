 ...
 * Page class
 */
class MachineList extends Machine
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MachineList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "MachineAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "MachineDelete";
        $this->MultiUpdateUrl = "MachineUpdate";
    ...
    public function run()
    {
        ...
        $this->repairCount->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->machine_category_id->AdvancedSearch->toJson(), ","); // Field machine_category_id
        ...
        $filterList = Concat($filterList, $this->repairCount->AdvancedSearch->toJson(), ","); // Field repairCount
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmachinesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field machine_category_id
        $this->machine_category_id->AdvancedSearch->SearchValue = @$filter["x_machine_category_id"];
        $this->machine_category_id->AdvancedSearch->SearchOperator = @$filter["z_machine_category_id"];
        $this->machine_category_id->AdvancedSearch->SearchCondition = @$filter["v_machine_category_id"];
        $this->machine_category_id->AdvancedSearch->SearchValue2 = @$filter["y_machine_category_id"];
        $this->machine_category_id->AdvancedSearch->SearchOperator2 = @$filter["w_machine_category_id"];
        $this->machine_category_id->AdvancedSearch->save();
        ...
        // Field repairCount
        $this->repairCount->AdvancedSearch->SearchValue = @$filter["x_repairCount"];
        $this->repairCount->AdvancedSearch->SearchOperator = @$filter["z_repairCount"];
        $this->repairCount->AdvancedSearch->SearchCondition = @$filter["v_repairCount"];
        $this->repairCount->AdvancedSearch->SearchValue2 = @$filter["y_repairCount"];
        $this->repairCount->AdvancedSearch->SearchOperator2 = @$filter["w_repairCount"];
        $this->repairCount->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->machine_category_id); // machine_category_id
            $this->updateSort($this->brand_id); // brand_id
            $this->updateSort($this->model); // model
            $this->updateSort($this->repairCount); // repairCount
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id->setSort("");
                $this->machine_category_id->setSort("");
                $this->brand_id->setSort("");
                $this->model->setSort("");
                $this->repairCount->setSort("");
            }
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"machine\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmachinelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmachinelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("machine_category_id", $this->createColumnOption("machine_category_id"));
            $option->add("brand_id", $this->createColumnOption("brand_id"));
            $option->add("model", $this->createColumnOption("model"));
            $option->add("repairCount", $this->createColumnOption("repairCount"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmachinesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmachinesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmachinelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        $row = [];
        $row['id'] = null;
        $row['machine_category_id'] = null;
        $row['brand_id'] = null;
        $row['model'] = null;
        $row['repairCount'] = null;
        $row['photo'] = null;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->machine_category_id->TooltipValue = "";
            ...
            // repairCount
            $this->repairCount->LinkCustomAttributes = "";
            $this->repairCount->HrefValue = "";
            $this->repairCount->TooltipValue = "";
            ...
            if ($this->photo->UseColorbox) {
                ...
                $this->photo->LinkAttrs["data-rel"] = "machine_x" . $this->RowCount . "_photo";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmachinelist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmachinesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    public function pageLoad()
    {
        ...
        Language()->setPhrase("AddLink", "Add Machine");
        Language()->setPhraseClass("DeleteLink", "");
        Language()->setPhrase("DeleteLink", "Delete Machine");
        Language()->setPhraseClass("EditLink", "");
        Language()->setPhrase("EditLink", "Edit Machine");
        Language()->setPhraseClass("ViewLink", "");
        Language()->setPhrase("ViewLink", "View Machine");
    }
