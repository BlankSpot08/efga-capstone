 ...
 * Page class
 */
class EmployeemonthlyexpenseList extends Employeemonthlyexpense
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'employee monthly expense';

    // Page object name
    public $PageObjName = "EmployeemonthlyexpenseList";
    ...
    public $FormName = "femployeemonthlyexpenselist";
    ...
    public function __construct()
    {
        ...
        // Table object (employeemonthlyexpense)
        if (!isset($GLOBALS["employeemonthlyexpense"]) || get_class($GLOBALS["employeemonthlyexpense"]) == PROJECT_NAMESPACE . "employeemonthlyexpense") {
            $GLOBALS["employeemonthlyexpense"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "EmployeemonthlyexpenseAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "EmployeemonthlyexpenseDelete";
        $this->MultiUpdateUrl = "EmployeemonthlyexpenseUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'employee monthly expense');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("employeemonthlyexpense");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->expSubcategory->AdvancedSearch->toJson(), ","); // Field expSubcategory
        $filterList = Concat($filterList, $this->expCategory->AdvancedSearch->toJson(), ","); // Field expCategory
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "femployeemonthlyexpensesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->expSubcategory->AdvancedSearch->SearchValue = @$filter["x_expSubcategory"];
        $this->expSubcategory->AdvancedSearch->SearchOperator = @$filter["z_expSubcategory"];
        $this->expSubcategory->AdvancedSearch->SearchCondition = @$filter["v_expSubcategory"];
        $this->expSubcategory->AdvancedSearch->SearchValue2 = @$filter["y_expSubcategory"];
        $this->expSubcategory->AdvancedSearch->SearchOperator2 = @$filter["w_expSubcategory"];
        $this->expSubcategory->AdvancedSearch->save();

        // Field expCategory
        $this->expCategory->AdvancedSearch->SearchValue = @$filter["x_expCategory"];
        $this->expCategory->AdvancedSearch->SearchOperator = @$filter["z_expCategory"];
        $this->expCategory->AdvancedSearch->SearchCondition = @$filter["v_expCategory"];
        $this->expCategory->AdvancedSearch->SearchValue2 = @$filter["y_expCategory"];
        $this->expCategory->AdvancedSearch->SearchOperator2 = @$filter["w_expCategory"];
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->expSubcategory, $default, true); // expSubcategory
        $this->buildSearchSql($where, $this->expCategory, $default, true); // expCategory
        ...
        $this->buildSearchSql($where, $this->dateClosed, $default, true); // dateClosed
        $this->buildSearchSql($where, $this->validatedBy, $default, true); // validatedBy
        ...
        if (!$default && $this->Command == "search") {
            $this->expSubcategory->AdvancedSearch->save(); // expSubcategory
            $this->expCategory->AdvancedSearch->save(); // expCategory
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->expSubcategory;
        $searchFlds[] = &$this->expCategory;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->expSubcategory->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->expCategory->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"femployeemonthlyexpenselist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"femployeemonthlyexpenselist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("expSubcategory", $this->createColumnOption("expSubcategory"));
            $option->add("expCategory", $this->createColumnOption("expCategory"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"femployeemonthlyexpensesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"femployeemonthlyexpensesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="femployeemonthlyexpenselist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // expSubcategory
        if ($this->expSubcategory->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->expSubcategory->AdvancedSearch->SearchValue != "" || $this->expSubcategory->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // expCategory
        if ($this->expCategory->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->expCategory->AdvancedSearch->SearchValue != "" || $this->expCategory->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // expCategory
            $this->expCategory->ViewValue = $this->expCategory->CurrentValue;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="femployeemonthlyexpenselist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"femployeemonthlyexpensesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
