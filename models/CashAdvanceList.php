 ...
 * Page class
 */
class CashAdvanceList extends CashAdvance
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CashAdvanceList";
    ...
    public $FormName = "fcash_advancelist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "CashAdvanceAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "CashAdvanceDelete";
        $this->MultiUpdateUrl = "CashAdvanceUpdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->expSubcategory_id->AdvancedSearch->toJson(), ","); // Field expSubcategory_id
        $filterList = Concat($filterList, $this->budget_id->AdvancedSearch->toJson(), ","); // Field budget_id
        $filterList = Concat($filterList, $this->machine_id->AdvancedSearch->toJson(), ","); // Field machine_id
        $filterList = Concat($filterList, $this->dateReceived->AdvancedSearch->toJson(), ","); // Field dateReceived
        ...
        $filterList = Concat($filterList, $this->used->AdvancedSearch->toJson(), ","); // Field used
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcash_advancesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field expSubcategory_id
        $this->expSubcategory_id->AdvancedSearch->SearchValue = @$filter["x_expSubcategory_id"];
        $this->expSubcategory_id->AdvancedSearch->SearchOperator = @$filter["z_expSubcategory_id"];
        $this->expSubcategory_id->AdvancedSearch->SearchCondition = @$filter["v_expSubcategory_id"];
        $this->expSubcategory_id->AdvancedSearch->SearchValue2 = @$filter["y_expSubcategory_id"];
        $this->expSubcategory_id->AdvancedSearch->SearchOperator2 = @$filter["w_expSubcategory_id"];
        $this->expSubcategory_id->AdvancedSearch->save();

        // Field budget_id
        $this->budget_id->AdvancedSearch->SearchValue = @$filter["x_budget_id"];
        $this->budget_id->AdvancedSearch->SearchOperator = @$filter["z_budget_id"];
        $this->budget_id->AdvancedSearch->SearchCondition = @$filter["v_budget_id"];
        $this->budget_id->AdvancedSearch->SearchValue2 = @$filter["y_budget_id"];
        $this->budget_id->AdvancedSearch->SearchOperator2 = @$filter["w_budget_id"];
        $this->budget_id->AdvancedSearch->save();
        ...
        // Field used
        $this->used->AdvancedSearch->SearchValue = @$filter["x_used"];
        $this->used->AdvancedSearch->SearchOperator = @$filter["z_used"];
        $this->used->AdvancedSearch->SearchCondition = @$filter["v_used"];
        $this->used->AdvancedSearch->SearchValue2 = @$filter["y_used"];
        $this->used->AdvancedSearch->SearchOperator2 = @$filter["w_used"];
        $this->used->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->expCategory_id, $default, true); // expCategory_id
        $this->buildSearchSql($where, $this->expSubcategory_id, $default, true); // expSubcategory_id
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->expCategory_id;
        $searchFlds[] = &$this->expSubcategory_id;
        $searchFlds[] = &$this->budget_id;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->expSubcategory_id); // expSubcategory_id
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->expSubcategory_id->setSort("");
                $this->budget_id->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcash_advancelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcash_advancelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("expCategory_id", $this->createColumnOption("expCategory_id"));
            $option->add("expSubcategory_id", $this->createColumnOption("expSubcategory_id"));
            $option->add("dateReceived", $this->createColumnOption("dateReceived"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcash_advancesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcash_advancesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcash_advancelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // expCategory_id
            if ($this->expCategory_id->UseFilter && !EmptyValue($this->expCategory_id->AdvancedSearch->SearchValue)) {
                if (is_array($this->expCategory_id->AdvancedSearch->SearchValue)) {
                    $this->expCategory_id->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->expCategory_id->AdvancedSearch->SearchValue);
                }
                $this->expCategory_id->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->expCategory_id->AdvancedSearch->SearchValue);
            }

            // expSubcategory_id
            if ($this->expSubcategory_id->UseFilter && !EmptyValue($this->expSubcategory_id->AdvancedSearch->SearchValue)) {
                if (is_array($this->expSubcategory_id->AdvancedSearch->SearchValue)) {
                    $this->expSubcategory_id->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->expSubcategory_id->AdvancedSearch->SearchValue);
                }
                $this->expSubcategory_id->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->expSubcategory_id->AdvancedSearch->SearchValue);
            }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fcash_advancelist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcash_advancesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    public function pageLoad()
    {
        ...
        Language()->setPhrase("AddLink", "Add Cash Advance");
        Language()->setPhraseClass("DeleteLink", "");
        Language()->setPhrase("DeleteLink", "Delete Cash Advance");
        Language()->setPhraseClass("EditLink", "");
        Language()->setPhrase("EditLink", "Edit Cash Advance");
        Language()->setPhraseClass("ViewLink", "");
        Language()->setPhrase("ViewLink", "View Cash Advance");
    }
